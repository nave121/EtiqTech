...
| Art. 16 | Reuse only if the prior procedure was mild or moderate, general health fully restored, veterinary advice taken, and the further procedure is mild, moderate or non-recovery (Art. 16(1)); one exceptional reuse after a severe procedure with competent-authority approval (Art. 16(2)) | `reuse:justification`, `surgery:multiple-survival` | candidate **EU-REUSE-CONDITIONS**: prior severity ≤ moderate, vet advice recorded, further procedure ∈ {mild, moderate, non-recovery}; severe-prior only with an authority approval reference |
...
3. `eu:reuse-conditions` — reuse permitted only if prior severity ≤ moderate, general health restored, veterinary advice recorded, and the further procedure is mild/moderate/non-recovery; severe-prior reuse needs an authority approval reference (Art. 16(1)–(2)).
