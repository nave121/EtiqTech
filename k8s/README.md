...
2. **The image.** CI job `publish` pushes `ghcr.io/nave121/etiqtech:main` and `:sha-<short>` on every green push to `main`. The package is private by default: either make it public (GitHub → Packages → etiqtech → Package settings → Change visibility) or add an `imagePullSecret` to the ServiceAccount in `base/rbac.yaml`. Pin `newTag` in `overlays/prod-remote/kustomization.yaml` to a `sha-` tag for reproducible rollouts; `main` floats.
