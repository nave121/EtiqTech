...
2. **The image.** CI job `publish` pushes `ghcr.io/nave121/etiqtech:main` and `:sha-<short>` on every green push to `main`. First push needs GitHub to let the workflow token write packages: repository → Settings → Actions → General → Workflow permissions → *Read and write permissions* (the first run failed with `permission_denied: write_package` until this is set); if a package named `etiqtech` already exists under the user, add this repository to it with write access instead. The package is then private by default: either make it public (GitHub → Packages → etiqtech → Package settings → Change visibility) or add an `imagePullSecret` to the ServiceAccount in `base/rbac.yaml`. Pin `newTag` in `overlays/prod-remote/kustomization.yaml` to a `sha-` tag for reproducible rollouts; `main` floats, and a new image under the same tag does not restart the pod by itself (`kubectl -n etiqtech rollout restart deploy/etiqtech`).
